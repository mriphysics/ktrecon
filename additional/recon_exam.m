function recon_exam( fcmrNo, seriesNos, rawDataDirPath, patchVersion, isGeoCorrn, maskDirPath, cusTrnDirPath, isHarmonicFilter, outputDirPath )
...
for seriesNo = seriesNos
    ...
    if ( isMask )
        ...
        % Custom Training Data
        if ~( isCusTrnData )
            mrecon_kt( rawDataFilePath, 'senseref', senseRefFilePath, 'coilsurvey', coilSurveyFilePath, 'outputdir', outputDirPath, 'outputname', idStr, 'patchversion', patchVersion, 'reconoptionpairs', reconOpts, 'mask', mask, 'makeHarmonicFilter', makeHarmonicFilter )
        elseif ( isCusTrnData )
            cusTrnMatFilePath = fullfile( cusTrnDirPath, strcat( ['s' num2str(seriesNo), '_hrh_recon.mat'] ) );  
            fprintf( 'custom training data file:       %s\n', cusTrnMatFilePath );
            mrecon_kt( rawDataFilePath, 'senseref', senseRefFilePath, 'coilsurvey', coilSurveyFilePath, 'outputdir', outputDirPath, 'outputname', idStr, 'patchversion', patchVersion, 'reconoptionpairs', reconOpts, 'mask', mask, 'cusTrnDirPath', cusTrnDirPath, 'makeHarmonicFilter', makeHarmonicFilter )
        end
