...
if ~rec.Par.Mine.Proce
    ...
    if rec.Alg.EstimGFact(1) && isfield(rec,'G') && isfield(rec,'M') && rec.Dyn.Typ2Wri(9)
        ...
        else error('Unknown PE configuration. Number of reformatted masks: %d / Number of PEs: %d',size(rec.S,5),length(rec.Par.Mine.pedsUn));
        end  
...
for n=typ2Rec';datTyp=rec.Plan.Types{n};datName=rec.Plan.TypeNames{n};    
    if rec.Dyn.Typ2Wri(n)
        ...
        if n==7 && rec.Alg.NoiseStand && rec.Par.Mine.Modal~=2;rec.(datTyp)=standardizeCoils(rec.(datTyp),rec.N,1);end%Coil de-standardization        
        x{c}=rec.(datTyp);MSV{c}=MS;
        if ~ismember(n,24:25) && isfield(rec.Enc,'ROI');MTV{c}=MTROI;else MTV{c}=MT;end
