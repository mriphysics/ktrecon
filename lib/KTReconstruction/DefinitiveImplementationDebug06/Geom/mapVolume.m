function xo=mapVolume(xi,y,MTx,MTy,addGrid,extGrid)

%MAPVOLUME   Maps the volume x into the geometry of volume y
%   XO=MAPVOLUME(XI,Y,MTX,MTY,{ADDGRID},{EXTGRID})
...
%   * ADDONE serves to add a value to the grid (1 in ReconFrame format 
...
for n=1:NXI(4);xo=dynInd(xo,n,4,interpn(sGrid{1},sGrid{2},sGrid{3},dynInd(xi,n,4),dGrid{1},dGrid{2},dGrid{3},'linear',0));end
xo=reshape(xo,NXO);
