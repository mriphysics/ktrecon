function svr=svrCNNFinalBrainDetection(svr)
...
%   * SVR is a svr structure containing different views (svr.y), spacings (svr.MS), 
%   orientations (svr.MT) and reconstruction parameters (svr.rec)
%   THIS FUNCTION IS AN ATTEMPT TO USE THE ACQ CNN TO DETECT IN RECON 
%   SPACE, BUT IT DOES NOT WORK
%
...
MS=svr.ParSVR.MS*ones(1,3);

svr.Elx=zeros([size(svr.x) 3],'like',svr.x);
for v=1:3
    %LOWRES ESTIMATE
    x=shiftdim(abs(svr.x),v);
    if gpu;x=gpuArray(x);end
    NX=size(x);
    NXRes=round(NX.*MS/networkRes);
    ...
    w=morphFourier(w,dist*ones(1,3),MS,ones(1,3),1);
    w=resampling(w,NX,[],2*ones(1,3));
    svr.Elx(:,:,:,v)=shiftdim(w,-v);
    %visSegment(svr.x,single(svr.Elx(:,:,:,v)>0.5),0);
end
svr.Elx=max(svr.Elx,[],4);
%svr.Elx=mean(svr.Elx,4);
svr.Elx=single(svr.Elx>0.5);


%if svr.ParSVR.Visualize;svrVisualization(svr.x,svr.Elx,'CNN brain detection');end
%svrWriteData(svr,'Elx',svr.Elx,1,[],'',0);
