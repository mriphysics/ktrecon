function [xh,sh,ph,amseh,Gamma,mseMatrix,amseMatrix,mse,amse,namse]=patchSVShrinkage(x,dimsM,sp,parR,cov,xGT,Ma,parV)
...
%   * {PARV} are parameters to control visualizations
...
if nargin<8 || isempty(parV);parV.visEmpiricalSamples=0;end
...
for ns=1:Nsweeps
    ...
    if ns==Nsweeps;fprintf('Size covariance:%s\n',sprintf(' %d',NC));end
    icprs=[];
    ...
    if parR.ESDMeth==2;bS=ceil(4096/(NC(4)*M));else bS=ceil(256/NC(4));end%MODIFICATIONS MAY BE REQUIRED FOR DIFFERENT COMPUTING PLATFORMS---THIS SEEMS A GOOD TRADE-OFF
    %if parR.ESDMeth==2;bS=ceil(8192/(NC(4)*M));else bS=ceil(512/NC(4));end%MODIFICATIONS MAY BE REQUIRED FOR DIFFERENT COMPUTING PLATFORMS
    amseGamma(:,ns)=0;
    contO=0;
    if parR.Verbosity>=3
        if gpu;wait(dev);end
        ...
        if ~isempty(vO)
            ...
            if ~isempty(cov) && isfield(cov,'Lambda')
                ...
                if strcmp(parR.NoiEstMeth,'None') || parR.ESDMeth==3
                    ...
                    %if parV.visEmpiricalSamples;visEmpiricalSamples(cov,esd,Beta);end%This may need modifications
                else
                    esd=[];
                end                
                if parR.Verbosity>=3
