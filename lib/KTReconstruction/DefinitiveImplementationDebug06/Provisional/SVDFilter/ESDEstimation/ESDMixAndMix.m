...
for l=1:NL%For each resolution level  
    ...
    %%GRID CLIPPING TO THE ESD LIMITS
    %if l==1
    %    [dens,gridx]=parUnaFun({dens,gridx},@permute,perm);
    %    BetaLim=arraySupport(dens,tolEnd,2);
    %    [dens,BetaLim,gridx]=parUnaFun({dens,gridx(BetaLim),gridx},@permute,perm);  
    %end
