function esd=ESDQuest(C,N,tolMinEig,tolUnique)
...
%   ESD=ESDQUEST(C,N,{TOLMINEIG},{TOLUNIQUE})
...
if nargin<4 || isempty(tolUnique);tolUnique=0;end
...
%POTENTIALLY GROUP EIGENVALUES AND ASSIGN THE VALUES TO THE QUEST STRUCTURE, BUT IT SEEMS THIS METHOD DOES NOT HANDLE THIS SITUATION DIRECTLY, SO
%I'VE DISABLED IT
%[tau,~,itol]=uniquetol(eigvo,tolUnique*max(eigvo));
%weight=accumarray(itol,w);
%esd{o}=callQuest(tau,weight,N,Beta);
...
function esdo=callQuest(tau,weight,N,Beta)
    ...
    if any(def.tau(:)>1e-6)
        ...
        den.f=den.f(idx);
        [esdo.grid,esdo.dens,esdo.thre]=parUnaFun({den.x,den.f,max(sup.endpoint(:))},@single);
