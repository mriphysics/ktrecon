...
% %     %OLD VERSION BEFORE ACCELERATION OF THE INNER LOOP
% %     cont=1;
% %     for q=1:length(subxN)    
% %        subxNq=subxN(q);
% %        vI=single(((1:subxNq-1)/(subxNq)));        
% %        xNo(cont:cont+subxNq-2)=sum(bsxfun(@times,xoi(:,q),[1-vI;vI]),1);
% %        cont=cont+subxNq-1;
% %     end
