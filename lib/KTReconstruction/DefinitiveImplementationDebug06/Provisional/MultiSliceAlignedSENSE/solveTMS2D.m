function [T,w,flagw,outlD]=solveTMS2D(x,y,T,H,S,Ak,xkGrid,kkGrid,kGrid,nT,w,flagw,NzSlab,outlP,thplc,BlSz)
...
for n=1:nT
    ...
    for s=1:BlSz:NT(5)
        ...
        for m=1:NHe         
            ...
            if ~gpu
                dHe(m,vS,:)=permute(multDimSum(GG,[1 2 4 6]),[1 5 3 2 4]);        
            else
        ...
        for m=1:NT(7)             
            ...
            if ~gpu
                dH(m,vS,:)=permute(multDimSum(G{m},[1 2 4 6]),[1 5 3 2 4]);                       
            else
