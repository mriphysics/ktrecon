function [x,T,outlD]=optimizeLevelMS2D(x,y,T,S,W,Ak,xGrid,parX,debug,res,estT,outlD,alpha)
...
Hsmooth=buildFilter([1 1 NW(3)],'2ndFiniteDiscrete',[1 1 1],gpu);
...
for n=1:nExtern       
    ...
    x=solveXMS2D(x,y,W,T,H,Hsmooth,S,Precond,Ak,xkGrid,kGrid,nX,NzSlab,outlD,BlSz);
    ...
    if parX.correct>0 && estT
        if debug==2;tstart=tic;end
        BlSz=1;
        [T,w,flagw,outlD]=solveTMS2D(x,y,T,H,S,Ak,xkGrid,kkGrid,kGrid,nT,w,flagw,NzSlab,parX.outlP,parX.thplc,BlSz);
