...
for n=typ2Rec'
    if ~ismember(n,[5 12]);datTyp=rec.Plan.Types{n};rec.(datTyp)=permute(rec.(datTyp),perm);end
end
voxSiz=voxSiz(perm(1:3));parXT.apod=parXT.apod(perm(1:3));

%COIL ARRAY COMPRESSION AND RECONSTRUCTED DATA GENERATION
NX=size(rec.M);
if prod(NX(1:3))>rec.Dyn.MaxMem(2);rec.y=gather(rec.y);rec.M=gather(rec.M);rec.S=gather(rec.S);end
...
for m=1:2;Residuals=fftGPU(Residuals,m,gpuFIn)/sqrt(NY(m));end
...
for l=1:L    
    ...
    if l==L;S=[];y=[];end
    for n=1:2;yRes=fftGPU(yRes,n,gpuFIn)/sqrt(NYres(n));end
    ...
    [ySt,FSt,EH.We,E.iSt,E.nSt,E.mSt,nSa]=buildHarmonicSampling(yRes,EH.We,E.mSt,NStates);iSt{l}=cat(1,E.iSt{:});%Samples as cell arrays
    ...
    for n=1:nExtern
        ...
        %%REFINE MASK
        %if l==L && n==1 && parXT.refineMask
        %    parS.nErode=parS.nErode-6;parS.nDilate=parS.nDilate-6;
        %    CX.Ma=refineMask(xRes,parS,voxSiz);
        %    if ~gpu;rec.M=gather(CX.Ma);else rec.M=CX.Ma;end
        %end
        ...
        %Wezeromeannorm=(WeO-Wemea)/Westd;               
        %p=erfc(Wezeromeannorm/sqrt(2))/2;%Strict p-value
        We=WeO/Wemea;
        We=1./We;  
        ...
        %if leff>0% && estT(l)==0
            subT(:)=1;
            if rec.Alg.AlignedRec~=4;outlWeV=find(outlWe);else outlWeV=1:length(outlWe);end
            ...
            if ismember(rec.Alg.AlignedRec,2:3)
        ...
        %end
        %if leff>0 && estT(l)~=0
        %    xRes=CSsolver(yX,E,EH,P,CX,R,xRes,nX,tolType{estT(l)+1},tol(estT(l)+1),rec.Dyn.Debug,1);          
        %    EH.We=[];
        %elseif leff>0 && estT(l)==0
            if rec.Alg.WriteSnapshots;visSegment(flip(permute(dynInd(xRes,E.nF,3),[2 1 3]),2),[],2,1,[],[],sprintf('Corrected ($l=$%d)',l),strcat(folderSnapshots,filesep,'Reconstructions'),strcat(fileName,rec.Plan.SuffOu,sprintf('_Di-l=%d',l)));end
            if parXT.computeCSRecon
                ...
                if rec.Alg.WriteSnapshots;visSegment(flip(permute(dynInd(xCS,E.nF,3),[2 1 3]),2),[],2,1,[],[],sprintf('Corrected regularized ($l=$%d)',l),strcat(folderSnapshots,filesep,'Reconstructions'),strcat(fileName,rec.Plan.SuffOu,sprintf('_Re-l=%d',l)));end
            end
            EH.We=[];
        end
